...
export function LiveGlobalSearchPreview() {
  ...
  return (
    <div data-mock-preview className="relative rounded-2xl bg-slate-900 border border-slate-700/80 shadow-2xl overflow-hidden select-none text-left font-sans max-h-[480px] flex flex-col">
      {/* Compact Top Bar */}
      <div className="flex items-center justify-between px-3.5 py-2 bg-slate-950/90 border-b border-slate-800 shrink-0">
        ...
        <div className="flex items-center gap-1 px-2.5 py-0.5 rounded bg-slate-900 border border-slate-800 text-[11px] font-medium text-slate-300">
          <span className="w-1.5 h-1.5 rounded-full bg-[#52A5CE] animate-pulse" />
          <span>Global Search (⌘K)</span>
        </div>
        <div className="text-[11px] font-mono text-slate-500">First Savvy</div>
      </div>
      ...
      <div className="p-3.5 sm:p-4 bg-slate-950/95 space-y-2.5 overflow-y-auto flex-1 custom-scrollbar">
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 rounded-xl bg-slate-900 border border-slate-700 text-base sm:text-xs text-white focus:outline-none focus:border-[#52A5CE] transition-colors"
          />
          {searchTerm && (
            <button
              type="button"
        ...
        <div className="flex flex-wrap items-center justify-between gap-1.5 text-[11px]">
          <div className="flex flex-wrap items-center gap-1">
            <span className="text-slate-500 font-bold uppercase text-[11px]">Quick:</span>
            ...
            {transactions.length > 0 && (
              <div className="space-y-1.5">
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-500">Plaid Sync</span>
                </div>
                  ...
                  {transactions.slice(0, 2).map((item) => (
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className="w-6 h-6 rounded bg-[#52A5CE]/10 text-[#52A5CE] flex items-center justify-center font-bold text-[11px] shrink-0 group-hover:bg-[#52A5CE] group-hover:text-white transition-colors">
                          {item.title.substring(0, 2).toUpperCase()}
                        </div>
                        <div className="min-w-0">
                          {/* §8: let the merchant name wrap on a phone instead of losing its tail */}
                          <div className="text-xs font-semibold text-white group-hover:text-[#52A5CE] transition-colors line-clamp-2 xl:line-clamp-none xl:truncate">{item.title}</div>
                          <div className="text-[11px] text-slate-400 line-clamp-2 xl:line-clamp-none xl:truncate">{item.subtitle}</div>
                        </div>
                      </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                  {otherResults.slice(0, 2).map((acc) => (
                    ...
                    >
                      <div className="text-xs font-bold text-white line-clamp-2 xl:line-clamp-none xl:truncate">{acc.title}</div>
                      <div className="text-[11px] text-slate-400 line-clamp-2 xl:line-clamp-none xl:truncate">{acc.subtitle}</div>
                      <div className="text-[11px] font-mono text-[#AACC96] font-bold mt-0.5">{acc.amount}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {selectedResult && (
          <div className="p-2 rounded-lg bg-slate-900 border border-[#52A5CE]/50 flex items-center justify-between text-[11px]">
            ...
            <button
              ...
              className="text-slate-400 hover:text-white text-[11px] font-semibold shrink-0 ml-2"
            >
              ✕
            </button>
          </div>
